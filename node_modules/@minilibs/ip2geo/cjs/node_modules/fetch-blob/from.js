...
var index$1 = require('../node-domexception/index.js');
...
// @ts-ignore
const fromBlob = (stat, path, type = '') => new index.Blob([new BlobDataItem({
  path,
  size: stat.size,
...
// @ts-ignore
const fromFile = (stat, path, type = '') => new file.File([new BlobDataItem({
  path,
  size: stat.size,
  lastModified: stat.mtimeMs,
  start: 0
...
class BlobDataItem {
  ...
  async * stream () {
    ...
    if (mtimeMs > this.lastModified) {
      throw new index$1('The requested file could not be read, typically due to permission problems that have occurred after a reference to a file was acquired.', 'NotReadableError')
    }
