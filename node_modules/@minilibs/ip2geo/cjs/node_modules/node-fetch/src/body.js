...
var index = require('../../fetch-blob/index.js');
var esm_min = require('../../formdata-polyfill/esm.min.js');
...
class Body {
	constructor(body, {
		...
		} else if (is.isBlob(body)) ; else if (node_buffer.Buffer.isBuffer(body)) ; else if (node_util.types.isAnyArrayBuffer(body)) {
			// Body is ArrayBuffer
			body = node_buffer.Buffer.from(body);
		} else if (ArrayBuffer.isView(body)) {
			// Body is ArrayBufferView
			body = node_buffer.Buffer.from(body.buffer, body.byteOffset, body.byteLength);
		} else if (body instanceof Stream) ; else if (body instanceof esm_min.FormData) {
			// Body is FormData
	...
	async formData() {
		...
		const {toFormData} = await Promise.resolve().then(function () { return require('./utils/multipart-parser.js'); });
